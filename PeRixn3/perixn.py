    ...
    for n in rxn_points: # Loop through each point being sampled in IRC/TRAJ
        ...
        for i in range(len(nics_coords)): # Loop through existing coordinates
            ...
            if arom_atoms[1] is None: #If only one fragment was specified
                ...
                rm=rotation_matrix_from_vectors(nvec, np.array([0,0,1]))
                rnvec=np.dot(rm,nvec)
                print("RNVEC", rnvec)
                ...
                rm=rotation_matrix_from_vectors(nvec1, np.array([0,0,1]))
        ...
        for nics_index in bq_points:
            ...
            nics_coords.append(['Bq',*nc])
            
        nics_coords_xyz = get_xyz_from_list(nics_coords)
        rnics_coords_xyz = rot_m(nics_coords_xyz,rm)
        
        #rewrite rotated coordinates
        nics_final=[]
        for i,v in enumerate(nics_coords):
            print(f'nics_coord={v}')
            new_v = [nics_coords[i][0],*rnics_coords_xyz[i]]
            nics_final.append(new_v)
...
def get_xyz_from_list(li):
    xyz=[]
    for l in li:
        xyz.append(np.asarray(l[1:]))
