...
def get_files_in_dir(cwd):
    ...
    if len(fchk_files)==0:
        print("No formatted checkpoint files found (.fchk)."
              "Use formchk to create .fchk from a .chk file")
...
def save_nics_data(workbook, bq_array, irc_step, bq_placement=np.arange(-5, 5.1, 0.5)):
    dim1 = len(bq_array)
    dim2 = len(bq_array[0])
...
def save_energies(ws, energies):
    ...
    ws.write(0,1, "Relative Electronic Energy (kcal/mol)")
...
def save_mcbo0(ws, mcbo):
    print(f'mcbo values: {mcbo}')
    ws.write(0,4,"nMCBO")
    for i,bo in enumerate(mcbo):
        ws.write(i+1, 4, mcbo[i])

def save_mnics0(ws, mnics):
    ws.write(0,3,"mnics")
    for i,mn in enumerate(mnics):
        ws.write(i+1,3,mnics[i])
        
def save_nics_with_energies(ws,wb,nics,mnics0,bq_placement=np.arange(-5,5.1,0.5)):
    leng=len(nics)
    wid=len(nics[0])
    
    for i in range(wid):
        ws.write(0, i + 5, 'NICS(%0.1f)zz' % bq_placement[i])
    ...
    for i in range(len(mnics0)):
        ...
        if mnics0[i] != None:
            print(mnics0[i])
            for j in range(wid):
                ws.write(i+1, j + 5, float(curr_nics_array[j]), num_format)
            print(f'c: {c}')
            c+=1
...
def run(ircfile):
    ...
    lfs, ffs = get_files_in_dir(os.getcwd())
    d={"IRC Step":[], "Energies":[], "mNICS":[], "nMCBO":[]}
    #dkeys=["IRC Step", "Energies", "mNICS", "nMCBO"]
    #d=dict.fromkeys(dkeys)#dictionary of irc step, energies, nics and mcbo
    ...
    #Copmuting ELF at CP's
    if calc_elf:
        delf=dict.fromkeys(['ELF', 'atom1', 'atom2', 'x', 'y', 'z', 'CP'])
        elfs=[]
        for i,f in enumerate(ffs):
            print("Calculating elfs...")
            line=get_elf(f).decode("utf-8")
            sline=line.splitlines()
            elfs.append(sline)
            print(f"Finished elf: {i}")
        
        
        wb_elf=xw.Workbook("elf_cp3.xlsx")
        #save_elf(wb_elf, elfs)
        save_bond_elf(wb_elf, elfs)
        wb_elf.close()
