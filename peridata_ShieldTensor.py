...
def compute_normal_vector(points):
    """
    Computes the normal vector most perpendicular to the plane formed by a set of 3D points using PCA.

    Args:
        points (list or np.ndarray): A 2D list or array of shape (n, 3) containing the XYZ coordinates of the points.

    Returns:
        np.ndarray: The normal vector (a 3D vector) most perpendicular to the plane.
    """
...
def get_nics(lfs,arom_atoms):
    ...
    for f in lfs:
        ...
        indices, bqlist = find_Bq_zz(f,arom_atoms) # Find Bq atoms
...
def get_mcbo(mcbo_scripts_path,arom_atoms,verbose=False):
    nmcbo_raw=None
    if verbose:
        print("Running MCBO command as: {} {}".format(mcbo_script_path, arom_atoms))
    try:
        nmcbo_raw = subprocess.check_output([mcbo_script_path, arom_atoms])
...
def find_Bq_zz(filename,arom_atoms):
    bq_list = []
    indices = []
    coordinates = extract_coordinates_from_gaussian_log(filename)
    with open(filename, 'r') as f:
        ...
        for i in range(len(lines)):
            ...
            if 'Bq   Isotropic' in line:
                shieldTensorString=[]
                shieldTensor=[]
                index = lines[i].strip().split()[5]
                for j in range(1,4):
                    curr_line=lines[i+j]
                    tensor_indices=[1,3,5]
                    split_line=curr_line.split()
                    shieldTensorString=([split_line[tensor_index] for tensor_index in tensor_indices])
                    shieldTensor.append([float(x) for x in shieldTensorString])
                zaligned=get_zaligned(arom_atoms,coordinates)
                zz=get_zz_component(shieldTensor,zaligned)
                bq_list.append(-float(zz))
                indices.append(index)
        f.close()
    return indices, np.asarray(bq_list)

def get_zaligned(arom_atoms,coords):
    if len(arom_atoms.split(' ')) == 1: #If only one fragment was specified
        arom_atoms=np.asarray([int(x)-1 for x in arom_atoms.split(',')])
        xyz=[]
        for index in arom_atoms:
            xyz.append(list(coords[index][1:]))  # Just xyz coords of aromatic carbons
        nvec = compute_normal_vector(xyz)  # vector normal to best fit plane to aromatic carbons
        return nvec
    
    else: # If two fragments were specified:
        frags=arom_atoms.split(' ')
        frag1=np.asarray([int(x)-1 for x in frags[0].split(',')])
        #frag2=np.asarray([int(x)-1 for x in frags[1].split(',')])
        xyz1=[]
        for index in frag1:
            xyz1.append(list(coords[index][1:]))  # Just xyz coords of aromatic carbons
        nvec1 = compute_normal_vector(xyz1)  # vector normal to best fit plane to aromatic carbons
        return nvec1
        
def get_zz_component(shielding_tensor, zaligned):
    """
    Rotates the shielding tensor to align with the zaligned vector and returns the zz component.

    Args:
        shielding_tensor (np.ndarray): A 3x3 shielding tensor.
        zaligned (np.ndarray): A 3D vector aligned with the desired z-axis.

    Returns:
        float: The zz component of the rotated shielding tensor.
    """
    # Normalize the zaligned vector
    zaligned_normalized = zaligned / np.linalg.norm(zaligned)

    # Define the original z-axis
    z_axis = np.array([0, 0, 1])

    # Compute the rotation axis (cross product of original z-axis and zaligned)
    rotation_axis = np.cross(z_axis, zaligned_normalized)
    rotation_axis_norm = np.linalg.norm(rotation_axis)

    if rotation_axis_norm < 1e-10:
        # If the rotation axis is zero, the vectors are already aligned
        rotation_matrix = np.eye(3)
    else:
        rotation_axis = rotation_axis / rotation_axis_norm

        # Compute the rotation angle (dot product of original z-axis and zaligned)
        rotation_angle = np.arccos(np.dot(z_axis, zaligned_normalized))
    ...
    # Apply the rotation to the shielding tensor
    rotated_shielding_tensor = np.dot(rotation_matrix.T, np.dot(shielding_tensor, rotation_matrix))

    # Extract the zz component
    zz_component = rotated_shielding_tensor[2, 2]

    return zz_component
    

def extract_coordinates_from_gaussian_log(log_file_path):
    """
    Extracts atomic coordinates from a Gaussian log file.

    Args:
        log_file_path (str): Path to the Gaussian log file.

    Returns:
        list: A list of tuples containing (atomic_number, x, y, z) coordinates.
    """
    coordinates = []
    with open(log_file_path, 'r') as file:
        log_content = file.read()

    # Use regex to find the "Standard orientation" section
    standard_orientation_section = re.search(
        r'Standard orientation:(.*?)Rotational constants', 
        log_content, 
        re.DOTALL
    )

    if not standard_orientation_section:
        # If "Standard orientation" is not found, look for "Input orientation"
        standard_orientation_section = re.search(
            r'Input orientation:(.*?)Rotational constants', 
            log_content, 
            re.DOTALL
        )
        if not standard_orientation_section:
    ...
    # Extract the table of coordinates
    coordinate_table = standard_orientation_section.group(1)

    # Use regex to extract atomic numbers and coordinates
    coordinate_lines = re.findall(
    ...
    for line in coordinate_lines:
        ...
        y = float(line[2])
        z = float(line[3])
        coordinates.append((atomic_number, x, y, z))
...
def save_nics_with_energies(ws,wb,nics,mnics0,bq_placement=np.arange(-3,3.001,0.1),verbose=False):
    wid=len(nics[0])
...
def run(ircfile,plot=True):
    ...
    if os.path.isfile("arom_atoms.txt"):
        with open("arom_atoms.txt","r") as file: arom_atoms=','.join(file.readlines())
    else:
        arom_atoms= input("Input indicies of aromatic atoms for nMCBO analysis (i.e 1,2,3,4,5,6).\n")
        with open("arom_atoms.txt","w") as file: file.write(arom_atoms)
    irc_step, indices, full_bq_array=get_nics(lfs,arom_atoms)
    ...
    mcbo=get_mcbo(mcbo_script_path,arom_atoms)
    ...
    if plot:
        ...
        flip=False
        pp.plot_data(np.asarray(column(energies, 1)), energiesKcalMol, np.asarray(mnics0,dtype=float), np.asarray(mcbo0), np.asarray((column(energies, 1))),flip=flip)
        pp.plotNICSHM(np.asarray(column(energies, 1)), energiesKcalMol,np.asarray(full_bq_array),irc_step,flip=flip)
